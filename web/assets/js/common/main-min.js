...
define('common/main',['bui/menu','bui/tab'],function(require) {
  ...
  function tabNav(moduleId,tabConfig,menuConfig,collapsed,homePage){
    ...
    menu.on('menuclick',function(ev){
      var item = ev.item;
      //swz  item.get('reload'),判断是否自动重载刷新
      if(item){
        _self.tab.addTab({id: item.get('id'), title: item.get('text'), href: item.get('href'),closeable : item.get('closeable')},item.get('reload'));
      }
  ...
  BUI.augment(mainPage,{
    ...
    _createModule:function(id){
      ...
      module = new tabNav(id,{render:tabId,height:getAutoHeight() - 5},
          {render:treeId,items:item.menu,height:getAutoHeight() - 5},item.collapsed,item.homePage);
      modules[id]= module;
